...
namespace LendingClubDotNet.Service
{
    ...
    //
    ///Ref. http://www.codeproject.com/Articles/692092/A-free-Export-to-Excel-Csharp-class-using-OpenXML
    public static class DocumentExportService
    {
        public static ResultObject CreateExcelDocument<T>(List<T> list, string xlsxFilePath)
        {
        ...
        public static ResultObject CreateExcelDocument(DataTable dt, string xlsxFilePath)
        {
            DataSet ds = new DataSet();
            ds.Tables.Add(dt);
            ResultObject result = CreateExcelDocument(ds, xlsxFilePath);
            ds.Tables.Remove(dt);
            return result;
        }
        #endregion
        ...
        public static ResultObject CreateExcelDocument(ListedLoansResponse listedLoansResponse
                                                    ...
                                                    , string fileFullName)
        {
            try
            {
                ///creating DataSet for Excel workbook, each table becomes a worksheet
                DataSet lendingClubDS = new DataSet("LendingClub");
                DataTable listedLoanTable = ListToDataTable<Loan>(listedLoansResponse.Loans);
                    //listedLoansResponse.Loans.ToDataTable();
                listedLoanTable.TableName = "Listed Loans";
                lendingClubDS.Tables.Add(listedLoanTable);

                DataTable noteTable = ListToDataTable<DetailedNote>(detailedNotesOwnedResponse.MyNotes);
                    //detailedNotesOwnedResponse.MyNotes.ToDataTable();
                for (int i = 0; i < portfoliosOwnedResponse.MyPortfolios.Count; i++)
                {
                    var portfolioName = portfoliosOwnedResponse.MyPortfolios[i].PortfolioName;
                    IEnumerable<DataRow> query = from detailedNote in noteTable.AsEnumerable()
                                                 where detailedNote.Field<string>("PortfolioName") == portfolioName
                                                 select detailedNote;

                    DataTable portfolioNoteTable = query.CopyToDataTable<DataRow>();
                    portfolioNoteTable.TableName = portfolioName;
                    lendingClubDS.Tables.Add(portfolioNoteTable);
                }

                ///Write dataset to Excel doc
                return CreateExcelDocument(lendingClubDS, fileFullName);
            }
            catch (Exception ex)
            {
                return new ResultObject(-1, ex.Message);
            }
        ...
        public static ResultObject CreateExcelDocument(DataSet ds, string excelFilename)
        {
            try
            {
                ...
                return new ResultObject(1, string.Empty);
            }
            catch (Exception ex)
            {
                return new ResultObject(-1, ex.Message);
